...
메인테이너가 2026-09-17 저장소 공개 전환과 **v0.2.2 공개 알파 릴리스**를 승인했습니다. GitHub에서는 pre-release로 구분하고 소스 ZIP과 검증 범위를 제공합니다. PyPI 배포나 Windows 독립 실행파일은 제공하지 않습니다. 수동 보고서 검증과 현업 정확도 평가가 남아 있으므로 안정판으로 소개하지 않습니다.

원본 데모가 포함된 main `105a0e1b5e67a45a299b07f4248cba46d5b6425c`의 [tests](https://github.com/ahillk9009-ship-it/ahill-cutguard/actions/runs/35210307887)와 [release-check](https://github.com/ahillk9009-ship-it/ahill-cutguard/actions/runs/35210307928)는 모두 통과했으며 전체 테스트·GUI·두 OS Chromium·패키징 로그를 확인했습니다.
...
후속 Windows 실제 사용 검증에서는 START_WINDOWS.bat을 호출하는 실행파일, 한글 GUI, 실제 영상·SRT·출력 폴더 선택창과 검사 완료를 확인했습니다. 저장된 완료 화면과 새 report.json의 후보 9개를 확인했고, 작업자가 기본 Edge 보고서 창의 자동 생성을 보고했습니다. 입력 영상·SRT 및 기존 보고서 2개의 작업 전후 SHA-256은 일치했습니다. 보고서 내용에 접근하기 전 Computer Use가 현재 URL을 충분히 확인할 수 없다는 보안 제한으로 중단됐습니다. 이 중단은 제품 오류로 확인된 것이 아닙니다. 실제 브라우저 화면·이미지·버튼·잘림 및 네이티브 검토 JSON 저장/불러오기·다른 보고서 거부는 미실행입니다.

- 자동 GUI 검증은 `askdirectory`, 오류 메시지 상자와 `webbrowser.open`을 모킹하며 입력 SRT 경로는 자동 입력합니다. 위 후속 실제 사용 검증과 자동 검증의 범위를 구분합니다.
...
- 자동 GUI·브라우저 smoke 검증은 합성 SRT 기반입니다. Windows FFmpeg 영상 통합은 로컬 전체 테스트에서, GUI 영상 검사 완료는 후속 데모 검증에서 확인했습니다. 실제 편집 영상의 정확도·시간 절감은 측정하지 않았습니다.
...
원본 파일 구성과 자동 검증, Windows 실행·선택·검사 완료의 근거를 확보했습니다. 현재까지 확인된 제품 오류는 없지만 미실행 항목이 있으므로 무결함을 의미하지 않습니다. 메인테이너 승인에 따라 제한을 명시한 공개 알파로 배포합니다. 보고서 화면·검토 파일의 수동 검증과 실사용 평가는 남아 있습니다.
